...
def initiate_payment(request):
    ...
    # if total_price <= 0:
    #     return JsonResponse({'error': 'Cart is empty or invalid total price'}, status=400)


    if request.method == 'POST':
        # Get user details and amount (e.g., from the cart or checkout form)
        # amount = int(request.POST.get('amount')) * 100  # Amount in paise (e.g., ₹500 -> 50000)
        amount = int(total_price * 100)  # ₹500 -> 50000 paise
        ...
        'payment_capture': '1'  # Auto-capture after payment success
          ...
          # Pass Razorpay details to the frontend
        context = {
        'razorpay_order_id': razorpay_order['id'],
        'razorpay_key': settings.RAZORPAY_KEY_ID,  # Public key for Razorpay Checkout
        'amount': amount,
        'currency': 'INR',
        }

        return render(request, 'standard_checkout.html', context)

   
        # Save the Razorpay order ID in your database (optional, for tracking)
        # request.session['razorpay_order_id'] = razorpay_order['id']
      

        # Redirect the user to Razorpay's hosted payment page
        # return redirect(f"https://checkout.razorpay.com/v1/checkout.js?order_id={razorpay_order['id']}")
...
def payment_success(request):
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect('billing_receipt')
